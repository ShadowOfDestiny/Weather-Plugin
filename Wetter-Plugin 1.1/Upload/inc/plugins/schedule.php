...
{
    global $mybb, $lang, $form; // Standard MyBB Globals

    // 1. Stelle sicher, dass $lang->schedule_permission existiert (wird im Code verwendet)
    // Wir versuchen nicht mehr, schedule.lang.php aktiv zu laden, um den "file not exist" Fehler zu vermeiden,
    // falls die Datei wirklich fehlt oder der Pfad nicht stimmt.
    // Stattdessen setzen wir einen Fallback-Wert, wenn die Variable nicht schon (z.B. durch einen anderen Hook) geladen wurde.
    if (!isset($lang->schedule_permission) || !is_string($lang->schedule_permission)) {
        $lang->schedule_permission = 'Terminplanung erlauben (Fallback)'; // Ein sinnvoller Standardtext
    }

    // 2. Stelle sicher, dass $lang->misc existiert und ein String ist (wird in der if-Bedingung verwendet)
    if (!isset($lang->misc) || !is_string($lang->misc)) {
        // Versuche, die Standard MyBB Admin misc.lang.php zu laden, da diese $lang->misc definieren könnte.
        // Die 'true' am Ende sagt, dass es eine Admin-Sprachdatei ist.
        // $lang->load() hat einen eingebauten Fallback auf Englisch.
        if (isset($lang) && method_exists($lang, 'load')) { // Sicherstellen, dass $lang ein Objekt ist und die Methode hat
             $lang->load('misc', false, true);
        }

        // Wenn $lang->misc nach dem Ladeversuch immer noch nicht existiert oder kein String ist,
        // setze es auf einen leeren String als Fallback.
        if (!isset($lang->misc) || !is_string($lang->misc)) {
            $lang->misc = '';
        }
    }

    // Original-Bedingung aus schedule.php, aber jetzt mit abgesicherten Sprachvariablen
    // und Prüfung auf isset($above['title'])
    if (isset($above['title']) && $above['title'] == $lang->misc && $lang->misc !== '') {
